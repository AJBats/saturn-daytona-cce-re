...
|------|----------|------:|---------------|----------------|--------|--------|
| 2026-06-11 | **Batch 1** (8 zero-inbound T_UNREACH_RETAIL_DEAD leaves): FUN_0602A8F8, FUN_0602AB90, FUN_0602C6C8, FUN_0602E91E, FUN_06030DEE, FUN_06030E32, FUN_06032918, FUN_06034C68 | 814 CSV / **808 net** (6 B alignment re-pad) | 0 (inbound_refs_pristine=0 each; grep of live shims + race_c.ld clean) | 0 in all three evidence legs (static, retail captures, fresh native sweep f9dc200c) | **SUCCESS** — `#ifndef MOD_TRANSPLANT` excludes in race.c; validate 8/8 + 4shift PASS; reloc invariants PASS; transplant race.bin 166,468 → 165,660 B; fixture re-created (`transplant_165660_threeseven_racestart.mzs`); rolling start + 843-frame poke lap clean, HUD/minimap live; 3 new REAL fires vs old lap capture (FUN_060295DE, FUN_0602E266, FUN_0602E372) all T_REACH_RETAIL_LIVE — benign ALIVE-set intel | 08d7ff3a |
| 2026-06-12 | **Batch 2** (69 T_UNREACH_RETAIL_DEAD subsegs; full remaining tier minus 5 chain-deferred rows falling into live code [0x0603FF64, 0x060400C4, 0x06044D64, 0x06044DA8, 0x06044ED4 — 204 B] and 3 label-referenced restores [FUN_0603631C, FUN_06036764, FUN_06036DA6 — 1,218 B; pools/bsr-strips referenced by surviving transplant-cut code]) | 5,992 CSV / **5,984 net** | 0 from compiled code (full label-closure scan incl. xref_/.L_pool_/.L_wpool_ classes, fixpoint-restored) | 0 in all three evidence legs | **SUCCESS after toolchain fix** — first attempt crashed the attract: 2-mod-4 shift broke pool-anchored braf dispatch tables (GAS pad between braf+4 and table; "ILLEGAL COMMAND" at relocated FUN_06045BA0 table interior; init exception screen). Fixed by the saturncc `.dispatch_table` construct migration (all 22 table instances) + pad-tripwire wrapper + braf_verify ground truth. transplant race.bin 165,660 → 159,676 B; 7 absorption pads approved in config/known_warnings.txt; rolling start + 843-frame poke lap clean twice (manual nav). **Fixture re-creation still owed for the 159,676 binary** (probe file `fresh_sweep/probes_transplant_159676.txt` is ready; old 165,660 fixture is stale). | 56ad0045 |
| 2026-06-16 | **Batch 3** (19 subsegs of the 0x06035xxx-0x06037xxx UNREACH dead cluster: 16 T_UNREACH_RETAIL_LIVE [060354A0, 06035624, 06035750, 06035904, 06035B0E, 06035B30, 06035C58, 06035C98, 06035EE8, 06035F48, 060366EC, 06036790, 06036CEC, 06036D52, 06036D7C, 06037200] + the 3 batch-2-deferred T_UNREACH_RETAIL_DEAD label-referenced rows now cleared [0603631C, 06036764, 06036DA6]) | **5,912 net** (CSV == net; no repad) | 0 from live code. Closure was iterative: build flagged `bsr FUN_06035C58` (in 0603631C), `bsr FUN_06036790` (in 06036764), `bra xref_060367E0` (in 06037200 jump table) -- all dead-to-dead refs; folding the whole closed component in resolved them. **Lesson: assembler `pcrel too far` / `displacement to undefined symbol` on a removal = a surviving function borrows a literal pool / branches into the cut block; the build is a free closure check (PC-relative 8-bit loads have no reloc type, so a dangling local pool ref surfaces as "too far", not "undefined").** | static legs only (no formal sweep yet) -- fresh probe sweep on the 159,672 binary still owed; PARSER_GAP residue + FUN_06038DD8 (April-live via pool jsr) + FUN_06038C64 (shared-pool, needs pool extraction) held back | **SUCCESS** -- `#ifndef MOD_TRANSPLANT` excludes in race.c; modwarn 7/7 pads (no change), free 8/8, align + portstamp PASS; transplant race.bin 165,584 -> **159,672 B** (headroom under retail 3,896 -> 9,808 B); interactive transplant boot/drive test clean (attract->select->race, car renders/drives, no collision/body-sway glitches). 4shift not re-run (transplant-only `#ifndef` change; non-mod path byte-identical per free 8/8). | 18fdb3eb |
| 2026-06-16 | **Batch 4** (rts-gate cascade): strip 5 rts-gate mod stubs to true stubs (FUN_0603C5CC, 06036A70, 0603E340, 0603E394, 0603E60C) + remove the 24-func position-integration cascade they were pinning (0603E14C/E7B0/EAAA/DF84/FAEA/36990/DD18/E0BA/DC9C/E3E8/FC10/DF28/FA54/E952/36948/368D4/F9E8/FA1E/36914/FBD0/E45C/E472/FAA8/FABE) + 4 PARSER_GAP SMC wrappers pulled in (E914->E952, EA6C->EAAA, FAD4, FBFC->FC10) | **4,172 net** | 0 from live code. Iterative closure surfaced: (1) rts-gate dead bodies were *falsely pinning* their call targets alive (static lexer sees body refs even after the gate rts) -- stripping to true stubs unpinned the subtree; (2) live registrar **FUN_0603E4A4 borrows FUN_0603E60C's shared literal pool** (.L_pool_0603E64C/650/654/658) -> preserved those 4 entries in the E60C stub (pool extraction); (3) the held PARSER_GAP set are **SMC dispatch-wrappers into the cascade** (E914/EA6C/FAD4/FBFC), referenced only by already-excluded code -> pulled in. **Kept as stubs (floor ~52 B): C5CC (live FUN_0603CE88 registers it as init per-frame callback), 36A70/E340/E394 (registered no-op callbacks), E60C (houses E4A4's borrowed pool).** | static gates + **interactive boot test PASS across tracks 1/2/3** (boot->select->race, car renders/drives, no init exception screen). Note: faked-velocity tick doesn't fire on tracks 2/3 -- pre-existing transplant-code behavior, NOT from this removal (cascade was dead/unreachable); deferred. | _this commit_ |
