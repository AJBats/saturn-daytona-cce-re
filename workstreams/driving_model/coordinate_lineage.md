...
| **Grass slowdown mechanism** | **FULLY CLOSED (ALL PCs CONFIRMED)** | COMPLETE CHAIN: polygon flags (0x0300=grass) → FUN_06036990 (point-in-polygon) → FUN_06036914 (surface props to scratch) → FUN_060386D8 (Y/banking) → **FUN_06036CEC (sub #1, PC 0x06036CD2) writes surface type to +0x4C-+0x58** (1→3→4 per wheel) → **FUN_06035B30 (sub #12 helper, confirmed PC 0x06035A3A) reads type, gates >2** → +0x70 grip clamped to [0x1999, 0x10000] → **sub #12: R10 drops 346× on grass** → net force reduced → speed plateaus at 64 km/h. All writer PCs confirmed by Explorer watchpoints. |
