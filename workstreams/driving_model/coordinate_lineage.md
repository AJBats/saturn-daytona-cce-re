...
| **Grass slowdown mechanism** | **CLOSED** | FULL CHAIN: polygon flags (0x0100=road, 0x0300=grass) → surface type written to +0x4C/+0x50/+0x54/+0x58 (sequential wheel contact, 1→3→4) → FUN_06035B30 (Ghidra FUN_0600DB30) reads surface type via iVar10 from +0x54-area field → gate: if surface_type > 2, takes grass path → +0x70 reduced, clamped to [0x1999, 0x10000] → sub #12 reads +0x70 as grip limit → net force reduced → speed plateaus. Confirmed: 0x1999 is the minimum grip on grass (10% of max). Explorer survey_001 update: R10 in sub #12 drops 346x on grass (0x39FC9→0x2AD) — the grass signal enters force formula entirely through R10. Polygon property D differs: pavement 0xFE1299EB vs grass 0x00066666. +0x04 and +0x70 converge to identical 0x1999 on grass (possible shared computation path). |
