...
|--------|------|---------------------------|------|---------|
| +48 | 4 | 0xFFB3EC0C | 77 | GBR+48 (passed to trig pipeline as r5) |
| +56 | 4 | 0xFF9E7FAF | 78 | GBR+56 (passed to trig pipeline as r7) |
| +76 | 4 | 0xFFFB6667 | 88 | GBR+76 (passed to normalization as r0/r1) |
| +146 | 2 | 0xAAC4 | Pipeline delay slot | GBR+146 (passed as R5→R11) |
| +16 | 2 | 0x2700 | 87 | GBR+16 (passed to FUN_060450F2 as r0) |

### Fields Written by EC86/ECB6

| Offset | Size | Value Before | Value After | Line | Notes |
|--------|------|-------------|-------------|------|-------|
| +146 | 2 | 0xAAC4 | 0xAB3F | 72 | GBR+146 written back (extu.w of r11) |
| +48 | 4 | 0xFFB3EC0C | 0xFFB64A40 | 93 | New GBR+48 from trig pipeline |
| +56 | 4 | 0xFF9E7FAF | 0xFF9D27A8 | 98 | New GBR+56 from trig pipeline |
...
- Result 1 → r1 (new R14+48 value)
- Result 3 → r3 (new R14+56 value)
Both include an `xtrct` (middle 32 bits of 64-bit MAC result) and an `add`
with a table value.

## Value Dynamics

### GBR+48 (written by EC86)
...
### GBR+146 (read and written by EC86)
...
### GBR+76 (read but NOT written by EC86)
...
| 1744-1745 | FUN_0603EAAA (tier 3) | After field[0x0E] computation |
...
| .L_pool_0603F08C | FUN_060450F2 | Line 86 | Trig computation (GBR+16) |
...
| FUN_0603ED00 | 0x0603ED00 | Variant of ECF0 with value computation from two inputs |
| FUN_0603EDC8 | 0x0603EDC8 | Tier-2 variant of EC86 for R14+48/52/56 (3D), uses EDF8 body |
| FUN_0603EDD6 | 0x0603EDD6 | Variant of EDC8 with value computation from two inputs |
...
not the player struct at 0x0605224C). Fields read/written are documented
in the GBR Fields Accessed section above. Called once per tier-3/4-5 car
per physics frame.
...
- The position deltas per physics frame are substantial:
  ...
  with entry 0 having larger deltas (entry 0's GBR+72 > entry 1's GBR+72).
