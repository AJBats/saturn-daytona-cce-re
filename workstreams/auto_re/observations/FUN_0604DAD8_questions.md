...
value_stable on GBR+0xE4 (globally static 0x10000) and GBR+0x74 (=0x002DD774,
unclear if globally static). Neither demonstrates function-specific behavior.

Can you run sample captures (idle + throttle) and analyze which fields this
function's address range [0x0604DAD8, 0x0604DB10) writes to? I need at least
one field with observed writes from a PC in the function's range to write a
