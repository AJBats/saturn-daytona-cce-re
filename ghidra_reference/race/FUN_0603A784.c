/* FUN_0603A784  0x0603A784 */


int FUN_0603a784(int *param_1,int *param_2,int param_3)

{
  bool bVar1;
  longlong lVar2;
  int iVar3;
  short sVar5;
  int iVar4;
  uint uVar6;
  int extraout_r1;
  int extraout_r2;
  uint uVar7;
  int iVar8;
  int extraout_r3;
  int extraout_r3_00;
  uint uVar9;
  int iVar10;
  bool bVar11;
  uint uVar12;
  ushort uVar13;
  undefined8 uVar14;
  
  if (param_2[9] < param_1[9]) {
    sVar5 = (*(code *)PTR_FUN_0603a864)(param_2[2] - param_1[2],*param_2 - *param_1);
    uVar13 = -sVar5;
    uVar9 = (uint)DAT_0603a862;
    uVar6 = param_1[0xf] - ((uint)uVar13 | uVar9 << 1);
    if ((int)uVar6 < 1) {
      uVar6 = -uVar6;
    }
    uVar6 = (uint)((uVar6 & 0xffff) < uVar9);
    func_0x0603ab14();
    iVar3 = extraout_r1;
    if (extraout_r1 >= (int)uVar9) {
      iVar3 = extraout_r1 - uVar9;
    }
    uVar12 = (uint)(extraout_r1 < (int)uVar9);
    bVar1 = uVar12 != uVar6;
    uVar14 = (*(code *)PTR_FUN_0603a868)(iVar3);
    uVar6 = DAT_0603a960;
    iVar3 = (int)uVar14;
    uVar7 = extraout_r2 - (short)uVar13 & 0xffff;
    if ((int)uVar9 <= (int)uVar7) {
      uVar7 = uVar7 - uVar9;
    }
    uVar9 = (uint)((ulonglong)uVar14 >> 0x29);
    bVar11 = false;
    if ((10 < uVar9) && (uVar9 < 0x37)) {
      bVar11 = true;
    }
    if (((bVar1) && (uVar12 = (uint)(uVar12 == 0), bVar11)) ||
       (param_3 = (*(int *)(uVar9 * 4 + DAT_0603a88c) - param_3) +
                  *(int *)((uVar7 >> 9) * 4 + DAT_0603a88c), 0 < param_3)) {
      uVar9 = param_1[0xe] - (int)*(short *)((int)param_2 + (int)DAT_0603a8c2) & 0xffff;
      if (((int)DAT_0603a8c4 < (int)uVar9) && ((int)uVar9 < DAT_0603a8c4 * 3)) {
        if (DAT_0603a8c8 < param_1[9] + param_2[9]) {
          *(uint *)(DAT_0603a8fc + 0x18) = *(uint *)(DAT_0603a8fc + 0x18) | DAT_0603a900;
          *(uint *)((int)param_1 + (int)DAT_0603a8f4) =
               *(uint *)((int)param_1 + (int)DAT_0603a8f4) | DAT_0603a904;
          *(int *)((int)param_1 + (int)DAT_0603a8f6) = (int)DAT_0603a8f8;
        }
      }
      else {
        if ((int)DAT_0603a960 < param_1[9] - param_2[9]) {
          *(uint *)(DAT_0603a964 + 0x18) = *(uint *)(DAT_0603a964 + 0x18) | DAT_0603a968;
          *(uint *)((int)param_1 + (int)DAT_0603a958) =
               *(uint *)((int)param_1 + (int)DAT_0603a958) | DAT_0603a96c;
          *(int *)((int)param_1 + (int)DAT_0603a95a) = (int)DAT_0603a95c;
        }
        if ((bVar11) || (param_1[9] - param_2[9] < (int)(uVar6 >> 1))) {
          uVar6 = param_1[0xe] - (int)(short)uVar13 & 0xffff;
          if (((int)uVar6 <= (int)DAT_0603a992) || (DAT_0603a992 * 3 <= (int)uVar6)) {
            if (bVar11) {
              *(int *)(DAT_0603aa10 + 0x10) = param_1[0xf];
            }
            iVar8 = (*(code *)PTR_FUN_0603aa14)(uVar6);
            lVar2 = (longlong)(int)((uint)(param_1[9] - param_2[9]) >> 2) * (longlong)iVar8;
            iVar8 = (int)((int)((ulonglong)lVar2 >> 0x20) << 0x10 | (uint)lVar2 >> 0x10) >> 1;
            lVar2 = (longlong)DAT_0603aa18 * (longlong)(param_2[9] + iVar8);
            *(uint *)(DAT_0603aa10 + 0x14) =
                 (int)((ulonglong)lVar2 >> 0x20) << 0x10 | (uint)lVar2 >> 0x10;
            param_1[9] = param_1[9] - iVar8;
          }
        }
        else {
          uVar6 = DAT_0603a970;
          if (uVar12 != 0) {
            uVar6 = DAT_0603a974;
          }
          param_1[0xc] = param_1[0xc] | uVar6;
        }
      }
      *(undefined2 *)((int)DAT_0603aa0c + (int)param_1) = 8;
      if (uVar12 == 0) {
        uVar6 = DAT_0603aa20;
        if (*(short *)(PTR_DAT_0603aa1c + *(char *)((int)DAT_0603aa0e + (int)param_1) * 2) != 0) {
          uVar6 = DAT_0603aa24;
        }
      }
      else {
        uVar6 = DAT_0603aa6c;
        if (*(short *)(PTR_DAT_0603aa1c + *(char *)((int)DAT_0603aa0e + (int)param_1) * 2) != 0) {
          uVar6 = DAT_0603aa70;
        }
      }
      param_1[0xc] = param_1[0xc] | uVar6;
      iVar8 = (int)DAT_0603aa68;
      if (uVar12 == 0) {
        iVar8 = iVar8 * 3;
      }
      iVar8 = (param_1[0xf] & 0xffffU) + iVar8;
      uVar6 = (int)((ulonglong)((longlong)param_3 * (longlong)iVar3) >> 0x20) << 0x10 |
              (uint)((longlong)param_3 * (longlong)iVar3) >> 0x10;
      if (bVar1) {
        uVar6 = uVar6 + DAT_0603aa74;
      }
      iVar3 = (*(code *)PTR_FUN_0603ab40)(-iVar8,*param_2,param_2[2]);
      iVar4 = (*(code *)PTR_FUN_0603ab44)(-iVar8);
      *(uint *)(DAT_0603ab48 + 8) =
           (int)((ulonglong)((longlong)extraout_r3 * (longlong)iVar4) >> 0x20) << 0x10 |
           (uint)((longlong)extraout_r3 * (longlong)iVar4) >> 0x10;
      lVar2 = (longlong)(int)((uVar6 >> 1) + (uVar6 >> 2)) * (longlong)iVar3;
      *(uint *)(DAT_0603ab48 + 0xc) = (int)((ulonglong)lVar2 >> 0x20) << 0x10 | (uint)lVar2 >> 0x10;
      iVar8 = iVar8 + DAT_0603ab4c;
      iVar4 = *param_1;
      iVar10 = param_1[2];
      iVar3 = (*(code *)PTR_FUN_0603ab40)(-iVar8);
      iVar8 = (*(code *)PTR_FUN_0603ab44)(-iVar8);
      *param_1 = iVar4 + ((int)((ulonglong)((longlong)extraout_r3_00 * (longlong)iVar8) >> 0x20) <<
                          0x10 | (uint)((longlong)extraout_r3_00 * (longlong)iVar8) >> 0x10);
      lVar2 = (longlong)(int)(uVar6 >> 2) * (longlong)iVar3;
      param_1[2] = iVar10 + ((int)((ulonglong)lVar2 >> 0x20) << 0x10 | (uint)lVar2 >> 0x10);
      iVar3 = (*(code *)PTR_FUN_0603ab50)(DAT_0603ab48);
    }
    return iVar3;
  }
  iVar3 = func_0x0603ab66(param_1,param_2,*param_2,param_2[2]);
  return iVar3;
}

