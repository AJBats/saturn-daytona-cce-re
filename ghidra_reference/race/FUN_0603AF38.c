/* FUN_0603AF38  0x0603AF38 */


int FUN_0603af38(void)

{
  bool bVar1;
  longlong lVar2;
  code *in_r0;
  short sVar4;
  int iVar3;
  uint uVar5;
  int extraout_r1;
  int iVar6;
  int extraout_r2;
  uint uVar7;
  int iVar8;
  int extraout_r3;
  int extraout_r3_00;
  uint uVar9;
  int iVar10;
  int iVar11;
  bool bVar12;
  uint uVar13;
  ushort uVar14;
  int unaff_r12;
  int *unaff_r13;
  int *unaff_r14;
  undefined8 uVar15;
  
  sVar4 = (*in_r0)();
  uVar14 = -sVar4;
  uVar9 = (uint)DAT_0603afd4;
  uVar5 = unaff_r14[0xf] - ((uint)uVar14 | uVar9 << 1);
  if ((int)uVar5 < 1) {
    uVar5 = -uVar5;
  }
  uVar5 = (uint)((uVar5 & 0xffff) < uVar9);
  FUN_0603ab14();
  iVar6 = extraout_r1;
  if (extraout_r1 >= (int)uVar9) {
    iVar6 = extraout_r1 - uVar9;
  }
  uVar13 = (uint)(extraout_r1 < (int)uVar9);
  bVar1 = uVar13 != uVar5;
  uVar15 = (*(code *)PTR_FUN_0603afdc)(iVar6);
  uVar5 = DAT_0603b0d4;
  iVar6 = (int)uVar15;
  uVar7 = extraout_r2 - (short)uVar14 & 0xffff;
  if ((int)uVar9 <= (int)uVar7) {
    uVar7 = uVar7 - uVar9;
  }
  uVar9 = (uint)((ulonglong)uVar15 >> 0x29);
  bVar12 = false;
  if ((10 < uVar9) && (uVar9 < 0x37)) {
    bVar12 = true;
  }
  if (((bVar1) && (uVar13 = (uint)(uVar13 == 0), bVar12)) ||
     (unaff_r12 = (*(int *)(uVar9 * 4 + DAT_0603b000) - unaff_r12) +
                  *(int *)((uVar7 >> 9) * 4 + DAT_0603b000), 0 < unaff_r12)) {
    uVar9 = unaff_r14[0xe] - unaff_r13[0xe] & 0xffff;
    if (((int)DAT_0603b030 < (int)uVar9) && ((int)uVar9 < DAT_0603b030 * 3)) {
      if (DAT_0603b034 < unaff_r14[9] + unaff_r13[9]) {
        *(uint *)((int)unaff_r14 + (int)DAT_0603b068) =
             *(uint *)((int)unaff_r14 + (int)DAT_0603b068) | DAT_0603b070;
        *(int *)((int)unaff_r14 + (int)DAT_0603b06a) = (int)DAT_0603b06c;
        *(uint *)((int)unaff_r13 + (int)DAT_0603b068) =
             *(uint *)((int)unaff_r13 + (int)DAT_0603b068) | DAT_0603b070;
        *(int *)((int)unaff_r13 + (int)DAT_0603b06a) = (int)DAT_0603b06c;
      }
    }
    else {
      if ((int)DAT_0603b0d4 < unaff_r14[9] - unaff_r13[9]) {
        *(uint *)((int)unaff_r14 + (int)DAT_0603b0cc) =
             *(uint *)((int)unaff_r14 + (int)DAT_0603b0cc) | DAT_0603b0d8;
        *(int *)((int)unaff_r14 + (int)DAT_0603b0ce) = (int)DAT_0603b0d0;
        *(uint *)((int)unaff_r13 + (int)DAT_0603b0cc) =
             *(uint *)((int)unaff_r13 + (int)DAT_0603b0cc) | DAT_0603b0d8;
        *(int *)((int)unaff_r13 + (int)DAT_0603b0ce) = (int)DAT_0603b0d0;
      }
      if ((bVar12) || (unaff_r14[9] - unaff_r13[9] < (int)(uVar5 >> 1))) {
        uVar5 = unaff_r14[0xe] - (int)(short)uVar14 & 0xffff;
        if (((int)uVar5 <= (int)DAT_0603b0fe) || (DAT_0603b0fe * 3 <= (int)uVar5)) {
          if (bVar12) {
            unaff_r13[0xf] = unaff_r14[0xf];
          }
          iVar8 = (*(code *)PTR_FUN_0603b164)(uVar5);
          lVar2 = (longlong)(int)((uint)(unaff_r14[9] - unaff_r13[9]) >> 2) * (longlong)iVar8;
          iVar8 = (int)((int)((ulonglong)lVar2 >> 0x20) << 0x10 | (uint)lVar2 >> 0x10) >> 1;
          unaff_r13[9] = unaff_r13[9] + iVar8;
          unaff_r14[9] = unaff_r14[9] - iVar8;
        }
      }
      else {
        uVar5 = DAT_0603b0dc;
        if (uVar13 != 0) {
          uVar5 = DAT_0603b0e0;
        }
        unaff_r14[0xc] = unaff_r14[0xc] | uVar5;
      }
    }
    *(undefined2 *)((int)DAT_0603b15e + (int)unaff_r14) = 8;
    if (uVar13 == 0) {
      uVar5 = DAT_0603b16c;
      if (*(short *)(PTR_DAT_0603b168 + *(char *)((int)DAT_0603b160 + (int)unaff_r14) * 2) != 0) {
        uVar5 = DAT_0603b170;
      }
    }
    else {
      uVar5 = DAT_0603b1b8;
      if (*(short *)(PTR_DAT_0603b168 + *(char *)((int)DAT_0603b160 + (int)unaff_r14) * 2) != 0) {
        uVar5 = DAT_0603b1bc;
      }
    }
    unaff_r14[0xc] = unaff_r14[0xc] | uVar5;
    iVar8 = (int)DAT_0603b1b4;
    if (uVar13 == 0) {
      iVar8 = iVar8 * 3;
    }
    iVar8 = (unaff_r14[0xf] & 0xffffU) + iVar8;
    uVar5 = (int)((ulonglong)((longlong)unaff_r12 * (longlong)iVar6) >> 0x20) << 0x10 |
            (uint)((longlong)unaff_r12 * (longlong)iVar6) >> 0x10;
    if (bVar1) {
      uVar5 = uVar5 + DAT_0603b1c0;
    }
    iVar10 = *unaff_r13;
    iVar11 = unaff_r13[2];
    iVar6 = (*(code *)PTR_FUN_0603b23c)(-iVar8);
    iVar3 = (*(code *)PTR_FUN_0603b240)(-iVar8);
    *unaff_r13 = iVar10 + ((int)((ulonglong)((longlong)extraout_r3 * (longlong)iVar3) >> 0x20) <<
                           0x10 | (uint)((longlong)extraout_r3 * (longlong)iVar3) >> 0x10);
    lVar2 = (longlong)(int)((uVar5 >> 1) + (uVar5 >> 2)) * (longlong)iVar6;
    unaff_r13[2] = iVar11 + ((int)((ulonglong)lVar2 >> 0x20) << 0x10 | (uint)lVar2 >> 0x10);
    iVar8 = iVar8 + DAT_0603b244;
    iVar10 = *unaff_r14;
    iVar11 = unaff_r14[2];
    iVar3 = (*(code *)PTR_FUN_0603b23c)(-iVar8);
    iVar6 = (*(code *)PTR_FUN_0603b240)(-iVar8);
    *unaff_r14 = iVar10 + ((int)((ulonglong)((longlong)extraout_r3_00 * (longlong)iVar6) >> 0x20) <<
                           0x10 | (uint)((longlong)extraout_r3_00 * (longlong)iVar6) >> 0x10);
    lVar2 = (longlong)(int)(uVar5 >> 2) * (longlong)iVar3;
    unaff_r14[2] = iVar11 + ((int)((ulonglong)lVar2 >> 0x20) << 0x10 | (uint)lVar2 >> 0x10);
  }
  return iVar6;
}

