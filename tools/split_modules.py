...
def generate_asm(name, data, base_addr, confirmed, func_offsets, pool_xrefs, out_path):
    file_len  = len(data)
    end_addr  = base_addr + file_len
    func_set  = set(func_offsets)
    func_labels = {off: 'FUN_%08X' % (base_addr + off) for off in func_offsets}
    addr_note = '(confirmed)' if confirmed else '(PROVISIONAL -- verify with round-trip build)'

    with open(out_path, 'w', newline='\n') as f:
        f.write('/* ================================================================\n')
        f.write(' * %s.s  --  Daytona USA CCE module: %s\n' % (name, name))
        f.write(' *\n')
        f.write(' * AUTO-GENERATED by split_modules.py -- do not edit by hand yet.\n')
        f.write(' * Every byte pair is emitted via .byte directives to guarantee\n')
        f.write(' * a byte-identical round-trip.\n')
        f.write(' *\n')
        ...
        while i < file_len:
            ...
            if i + 1 < file_len:
                ...
                mnemonic = decode_sh2_insn(word, addr, data, base_addr)
                if mnemonic:
...
def process_module(name, rel_path, base_addr, confirmed, force=False):
    ...
    out_s    = os.path.join(out_dir, '%s.s' % name)
    out_syms = os.path.join(out_dir, '%s_syms.txt' % name)
    out_ld   = os.path.join(out_dir, '%s.ld' % name)

    if os.path.isfile(out_s) and not force:
        print('[%s]  skipped (already exists -- use --force to regenerate)' % name)
        return True
    ...
    generate_asm(name, data, base_addr, confirmed, func_offsets, pool_xrefs, out_s)
    generate_symbols(name, base_addr, func_offsets, out_syms)
    generate_linker_script(name, base_addr, confirmed, out_ld)

    asm_size = os.path.getsize(out_s)
    print('  Output: %s  (%.1f MB)' % (out_s, asm_size / (1024 * 1024)))
