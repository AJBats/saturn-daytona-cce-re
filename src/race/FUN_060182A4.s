/* FUN_060182A4  0x060182A4 */

    .section .text.FUN_060182A4
    .global FUN_060182A4
    .type FUN_060182A4, @function
FUN_060182A4:
    sts.l pr, @-r15
    shll r11
    add r2, r11
    exts.b r11, r11
    mov.b @(r0, r11), r11
    add #0x1, r11
    mov r11, r4
    cmp/ge r13, r4
    bt/s .L_060182C2
    add r3, r14
    mov.l .L_pool_06018380, r2
    mov.b @r2, r0
    cmp/eq #0x2, r0
    bf .L_060182C2
    add #0x14, r4
.L_060182C2:
    mov r4, r1
    mov.l .L_pool_0601838C, r8
    mov.l .L_pool_06018384, r9
    mov.l .L_pool_06018388, r10
    mov.l @r10, r3
    mov.l @r9, r2
    add #0x5, r3
    add #0x5, r2
    jsr @r8
    mov r13, r0
    add #0x30, r0
    mov.l .L_pool_06018390, r8
    mov r4, r1
    mov.b r0, @r2
    mov.b r0, @r3
    mov.l @r10, r3
    add #0x6, r3
    mov.l @r9, r2
    add #0x6, r2
    jsr @r8
    mov r13, r0
    mov.l .L_pool_06018394, r5
    add #0x30, r0
    mov.b r0, @r2
    mov.b r0, @r3
    mov.l .L_pool_06018398, r3
    jsr @r3
    mov.l @r10, r4
    mov.l .L_pool_06018394, r5
    mov.l .L_pool_0601839C, r2
    jsr @r2
    mov.l @r9, r4
    mov.l .L_pool_060183A0, r3
    mov.l .L_pool_060183A4, r2
    mov.l @r3, r6
    mov.l @r2, r5
    mov.l .L_pool_060183A8, r1
    mov.l .L_pool_060183AC, r3
    jsr @r3
    mov.l @r1, r4
    cmp/ge r13, r11
    bt .L_060183D8
    shll2 r12
    mov.l .L_pool_060183B4, r0
    mov.l .L_pool_060183B0, r3
    mov.w .L_wpool_06018372, r2
    mov.l @r3, r4
    mov.l .L_pool_060183B8, r1
    mov.l r4, @(r0, r12)
    mov.w r2, @r4
    mov.l @r1, r0
    mov.l .L_pool_060183BC, r2
    mov.w @(2, r0), r0
    mov.l .L_pool_060183C0, r1
    mov.w r0, @(2, r4)
    mov.w @r2, r0
    mov.w r0, @(4, r4)
    mov.w .L_wpool_06018374, r0
    mov.w r0, @(6, r4)
    mov.l @r1, r0
    mov.l r0, @r14
    mov.l .L_pool_060183C4, r2
    add #0x4, r14
    mov.l .L_pool_060183C8, r1
    mov.l @r2, r0
    mov.l .L_pool_060183CC, r2
    mov.l r0, @r14
    mov.l @r1, r0
    add #0x4, r14
    mov.l r0, @r14
    add #0x4, r14
    mov.l @r2, r0
    mov.l r0, @r14
    add #0x4, r14
    mov.l .L_pool_060183D0, r4
    mov.l @r4, r0
    mov.l r0, @r14
    mov.l @r4, r0
    add #0x4, r14
    mov.l r0, @r14
    add #0x4, r14
    mov.l @r4, r0
    mov.l r0, @r14
    mov.l .L_pool_060183D4, r0
    add #0x4, r14
    mov.l @r0, r3
    bra .L_06018406
    mov.l r3, @r14
.L_wpool_06018372:
    .byte 0x14, 0x88  /* 06018372: mov.l r8,@(0x20,r4) */
.L_wpool_06018374:
    .byte 0x04, 0x20  /* 06018374: .word 0x0420 */
    .byte 0xFF, 0xFF  /* 06018376: .word 0xFFFF */
    .4byte sym_06056578  /* 06018378 = 0x06056578 */
    .4byte sym_060540B6  /* 0601837C = 0x060540B6 */
.L_pool_06018380:
    .4byte sym_002FC233  /* 06018380 = 0x002FC233 */
.L_pool_06018384:
    .4byte sym_0604EC50  /* 06018384 = 0x0604EC50 */
.L_pool_06018388:
    .4byte sym_0604EC4C  /* 06018388 = 0x0604EC4C */
.L_pool_0601838C:
    .4byte FUN_06008A5C  /* 0601838C = 0x06008A5C */
.L_pool_06018390:
    .4byte sym_0604C88C  /* 06018390 = 0x0604C88C */
.L_pool_06018394:
    .4byte sym_060565D0  /* 06018394 = 0x060565D0 */
.L_pool_06018398:
    .4byte sym_06048278  /* 06018398 = 0x06048278 */
.L_pool_0601839C:
    .4byte sym_0604828C  /* 0601839C = 0x0604828C */
.L_pool_060183A0:
    .4byte sym_060568B4  /* 060183A0 = 0x060568B4 */
.L_pool_060183A4:
    .4byte sym_060568B8  /* 060183A4 = 0x060568B8 */
.L_pool_060183A8:
    .4byte sym_0605666C  /* 060183A8 = 0x0605666C */
.L_pool_060183AC:
    .4byte sym_060482F8  /* 060183AC = 0x060482F8 */
.L_pool_060183B0:
    .4byte sym_06056670  /* 060183B0 = 0x06056670 */
.L_pool_060183B4:
    .4byte sym_060565BC  /* 060183B4 = 0x060565BC */
.L_pool_060183B8:
    .4byte sym_060565D4  /* 060183B8 = 0x060565D4 */
.L_pool_060183BC:
    .4byte sym_060565C8  /* 060183BC = 0x060565C8 */
.L_pool_060183C0:
    .4byte sym_06056680  /* 060183C0 = 0x06056680 */
.L_pool_060183C4:
    .4byte sym_06056678  /* 060183C4 = 0x06056678 */
.L_pool_060183C8:
    .4byte sym_0605667C  /* 060183C8 = 0x0605667C */
.L_pool_060183CC:
    .4byte sym_06056674  /* 060183CC = 0x06056674 */
.L_pool_060183D0:
    .4byte sym_06056688  /* 060183D0 = 0x06056688 */
.L_pool_060183D4:
    .4byte sym_06056684  /* 060183D4 = 0x06056684 */
.L_060183D8:
    .byte 0xD1, 0x40  /* 060183D8: mov.l @(0x100,PC),r1  {[0x060184DC] = 0x0605669C} */
    mov.l @r1, r3
    .byte 0xD0, 0x40  /* 060183DC: mov.l @(0x100,PC),r0  {[0x060184E0] = 0x060566A0} */
    mov.l r3, @r14
    mov.l @r0, r3
    add #0x4, r14
    .byte 0xD1, 0x3F  /* 060183E4: mov.l @(0xFC,PC),r1  {[0x060184E4] = 0x060566A4} */
    mov.l r3, @r14
    mov.l @r1, r3
    add #0x4, r14
    .byte 0xD0, 0x3E  /* 060183EC: mov.l @(0xF8,PC),r0  {[0x060184E8] = 0x060566A8} */
    mov.l r3, @r14
    mov.l @r0, r3
    add #0x4, r14
    .byte 0xD1, 0x3D  /* 060183F4: mov.l @(0xF4,PC),r1  {[0x060184EC] = 0x060566AC} */
    mov.l r3, @r14
    mov.l @r1, r3
    add #0x4, r14
    mov.l r3, @r14
    add #0x4, r14
    .byte 0xD3, 0x3B  /* 06018400: mov.l @(0xEC,PC),r3  {[0x060184F0] = 0x060566B0} */
    mov.l @r3, r2
    mov.l r2, @r14
.L_06018406:
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14
